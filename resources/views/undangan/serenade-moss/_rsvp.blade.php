<div class="text-center py-20 px-6 bg-[#f9fafb]">
    <h2 class="text-3xl md:text-4xl font-semibold text-[#c0392b] mb-10 tracking-wide">Konfirmasi Kehadiran</h2>
    ...
    <form method="POST" action="{{ route('rsvp.store', $undangan->slug) }}" class="max-w-xl mx-auto bg-white/90 backdrop-blur-md p-6 md:p-8 rounded-xl shadow transition space-y-6">
        ...
        <input type="text" name="nama" placeholder="Nama Anda" required
            class="w-full p-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#c0392b] text-base md:text-lg placeholder-gray-500 transition"/>

        <textarea name="pesan" rows="4" placeholder="Ucapan atau Doa"
            class="w-full p-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#c0392b] text-base md:text-lg placeholder-gray-500 transition"></textarea>

        <button type="submit"
            class="w-full py-3 bg-[#c0392b] hover:bg-[#e74c3c] text-white font-semibold rounded-lg transition duration-300 text-lg">
            Kirim
        </button>
    </form>
            ...
            <div class="space-y-6 border-t border-gray-200 pt-6">
                @foreach($rsvps as $rsvp)
                    <div class="border-b border-gray-100 pb-4">
                        <span class="font-semibold text-[#c0392b]">{{ $rsvp->nama }}</span>
                        @if($rsvp->pesan)
